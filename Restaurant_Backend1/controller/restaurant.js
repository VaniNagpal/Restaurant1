...
exports.postRestaurant= ErrorWrapper(async (req,res,next)=>{
...
exports.postCusineCategoryAdd = ErrorWrapper(async (req, res, next) => {
    const { categories, restaurant_name } = req.body;
    const incomingFields = Object.keys(req.body);
    
    
    const requiredFields = ["categories","restaurant_name"];
    const missingFields = requiredFields.filter((field) => !incomingFields.includes(field));
    // To read image we need to use multer
    if (missingFields.length > 0) {
        // Status codes are necessary to throw errors
        throw new ErrorHandler(401, `Provide missing fields ${missingFields.join(',')} to add a cusine category`);
    }

    let newCusineCategories = categories.split(',');
    newCusineCategories = newCusineCategories.map(c => c.trim().toLowerCase());
    console.log(newCusineCategories);
...
exports.postdeleteCusineCategory= ErrorWrapper(async(req,res,next)=>{
    const{id}= req.params;
...
exports.postUpdateFoodItem = ErrorWrapper(async (req, res, next) => {
    const { id } = req.params;
...
exports.getDeleteFoodItem = ErrorWrapper(async (req, res, next) => {
    const { id } = req.params;
...
exports.getFoodItems = ErrorWrapper(async (req, res, next) => {
    const { restaurant_name, category } = req.query;
    ...
    try {
        ...
        const index = restaurant.cusines.findIndex((item) => item.category === category.toLowerCase());
        if (index == -1) {
...
exports.getFoodItem = ErrorWrapper(async (req, res, next) => {
    const { id } = req.params;
    const { restaurant_name, category } = req.query;
...
exports.getAllCusines = ErrorWrapper(async (req, res, next) => {
    const { id } = req.params;
    const { restaurant_name, category } = req.query;
...
exports.postAddFoodImage = ErrorWrapper(async (req, res, next) => {
    const { id } = req.params;
...
exports.getRestaurants = ErrorWrapper(async (req, res, next) => {
    try {
...
exports.getRestaurant = ErrorWrapper(async (req, res, next) => {
